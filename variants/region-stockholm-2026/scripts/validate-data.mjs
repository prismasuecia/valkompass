...
if (data.jurisdiction !== 'region-stockholm' || data.questions.length !== 6) throw new Error('Expected six draft Region Stockholm questions');
...
for (const question of data.questions) {
  ...
  ids.add(question.id);
  if (question.answerScale && !['agreement', 'tax-level'].includes(question.answerScale)) throw new Error(`Invalid answer scale: ${question.id}`);
