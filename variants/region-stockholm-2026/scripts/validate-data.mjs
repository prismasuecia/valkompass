...
for (const question of data.questions) {
  ...
  ids.add(question.id);
  if (!['agreement', 'tax-level', 'private-share', 'categorical'].includes(question.answerScale)) throw new Error(`Invalid answer scale: ${question.id}`);
  if (typeof question.scoringApproved !== 'boolean') throw new Error(`Missing review decision: ${question.id}`);
  if (!question.scoringApproved && !question.comparisonNote) throw new Error(`Missing exclusion notice: ${question.id}`);
  if (!question.sourceUrls?.length || question.sourceUrls.some(url => !url.startsWith('https://'))) throw new Error(`Missing source links: ${question.id}`);
  ...
  if (question.explanation.sections.length !== 3) throw new Error(`Expected three explanation sections: ${question.id}`);
